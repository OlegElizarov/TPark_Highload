...
|-----|-----|-----------|---------|-----------------|------------------|------------------|----|------|-------|
|bigint|bigint|bigint array[5]|varchar(50)|varchar(11000000)|timestamp|boolean|bigint|bigint|varchar(200)|

Максимальный вес одного письма - 11000331 байт ~ 10MB.
...
при среднем размере данных в 97KB показатели LevelDB равны:
* Random Reads - 19,245 ops/sec
* Random Writes - 126 ops/sec
* Batch Random Writes - 1,574 entries/sec

<br>А при размере в 100B и 128MB кеша:
...
**1)** Получения списка последних писем (50 штук).Данные будут браться из отдельной таблицы коротких сообщений. 
...
RPS писем c вложенияvb: 5 000 000 * 1 / 24 * 60 * 60 = 58 
```
Видно, что один сервер может не вытянуть нагрузку по записи писем.Заметно, что batch write намного превосходит 
одиночную запись.А учитывая , что с точки зрения бизнеса у нас нет жесткой необходимости в моментальной доставке
писем до получателя,алгоритм наших действий будет таков:
* Определяем получателя письма и наличие в нем вложений
* По получателю письма определяем сервер(Через запрос в Redis) , на котором хранятся его данные
* Заводим сервер очередей по типу RabbitMQ , в котором группируем очереди по серверам назначения
* Раз в 2 секунды(или по достижению лимита) мы отправляем всю очередь в ее сервер одним большим 
батчем, что повысит эффективность записи.
<br>
...
данные,то он готов подождать чуть дольше.Перенос писем из "горячих" серверов в "холодные" будет осуществляться 
два раза в год в течение нескольких дней.
...
Пускай пользователь уже зарегистрирован.Рассматривать будет только позитивные сценарии.<br>
...
**5)** Бэкенд сообщений узнает у Redis'a в какой сервер БД направляется это сообщение и добавляет его в 
соответствующую очередь в RabbitMQ.<br>
**6)** Сервер очередей собирает очереди на групповую запись в определенные сервера БД.Либо раз в 2 секунды,
либо по достижению лимита он полностью скармливает всю кучу сообщений в сервер БД.<br>
**7)** Сервер БД получает письма группами, что способствует более эффективной вставки.Сообщения юзера 
с ID=IDi он сохраняет в папочку юзера IDi на этом диске.Так же обновляется файл, где лежит список 
последних коротких сообщений для этого юзера и прочие счетчики в этой папке.<br>
...
| CPU(cores) | RAM(GB) | SSD(GB)/RAID(0) |
|------------|---------|---------|
| 8         | 32      | 4096 x 4    |


то на один из них с учетом запаса под рост данных в 50% на нем уместится 8192GB/79MB ~ **106 000  пользователей**.
Всего таких серверов понадобится 80 000 000 / 106 000 = **753 штуки**
Надежность этих данных требуется высокая, поэтому нам придется делать по 2 реплики к каждому.
**Итого 1506 реплик, 2260 сервера всего.**
...
На такой сервер с учетом запаса данных в 66% на нем уместится 6TB/170MB ~ **37 000  пользователей**.
Всего таких серверов понадобится 80 000 000 / 37 000 = **2 161 штука**
На холодные сервера не сильная нагрузка и не такие высокие требования к надежности, поэтому нам хватит 
по 1 реплики к каждому. **Итого еще 2 161 реплика , 4 322 сервера всего.**
...
Сервер RabbitMQ:<br>
Согласно [rabbitmq_bench](https://www.confluent.io/blog/kafka-fastest-messaging-system/)
пропускная способность 30MB/s.

| CPU(cores) | RAM(GB) | SSD(GB) |
|------------|---------|---------|
| 8         | 64      | 512    |

В пункте 4.3 мы посчитали, что получаем в среднем 1000GB писем в день.
Это 12MB писем в секунду.Как мы видим,у нас есть запас примерно в 2 раза.
Сервер очередей будет стоять только перед "горячими" серверами, лучше всего поставить по одному такому
перед каждыми 250 серверами БД, но так как важна надежность , то добавим каждому по одному запасному.
**Итого 6 серверов очередей.**
