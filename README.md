...
А так как их у нас их 10 штук(основных), то еще будем использовать L4 DNS балансировку.Алгоритм балансировки
DNS и некоторые его тонкости представлены на картинке выше.з
Используем алгоритм Round Robin,который позволит равномерно распределить нагрузку и на уровне DNS, и на 
уровне веб-серверов.
Так же отметим, что сам по себе nginx имеет инструменты для SSL. 
