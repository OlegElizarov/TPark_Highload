...
А при размере в 100B и 128MB кеша :
...
```
Видно, что один сервер может не вытянуть нагрузку по записи писем.Заметно, что batch write заметно превосходит 
...
В более быстрых хранилищах мы будем хранить эти свежые данные, а в более вместительных - более старые 
...
Пускай пользователь уже зарегистрирован.Рассматривать будет только позитивные сценарии.
**1)** Пользователь запрашивает наш сайт. Веб-сервер(о нем речь пойдет ниже) отдает ему необходимую 
статику,а потом перенаправляет запросы с заголовками на бэкенд.
...
получения отдает все обратно клиенту.
**3)** У клиента отобразилась стартовая страница со всеми данными.Он переходит в раздел написания письма.Создает 
его,прикрепляет файл и совершает отправку.
**4)** Отправка с вложениями по REST API методу отличается от отправки без вложения.Веб-сервер про это знает и 
отправляет запрос на бэкенд, отвечающий за сообщения с вложениями.
**5)** Бэкенд сообщений узнает у Redis'a в какой сервер БД направляются это сообщение и добавляет его в 
соответствующую очередь в RabbitMQ.
**6)** Сервер очередей собирает очереди на групповую запись в определенные сервера БД.Либо раз в 2 секунды,
либо по достижению лимита он полностью скармливает всю кучу сообщений в сервер БД.
...
последних коротких сообщений для этого юзера и прочие счетчики в этой папке.
...
(3600 * 4KB)(письма без вложений)(14400) + (400 * 400KB)(с вложениями)(160 000) = 174 400 KB ~ 170MB
```
